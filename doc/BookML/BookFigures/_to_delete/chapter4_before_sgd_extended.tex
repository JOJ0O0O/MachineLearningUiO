...
The word \emph{momentum} is not a metaphor.  Consider a particle of mass $m$
moving in the potential $C(\bm{\theta})$ -- the cost function read as a
landscape -- and subject to a friction force proportional to its velocity,
with friction coefficient $\mu$.  Newton's second law reads
\begin{equation}
  m\,\frac{d^{2}\bm{\theta}}{dt^{2}} + \mu\,\frac{d\bm{\theta}}{dt}
  = -\nabla C(\bm{\theta}) .
  \label{eq:4-heavyball}
\end{equation}
Without friction the particle would roll down the slope, overshoot the
bottom and oscillate forever; with friction it loses energy on every swing
and settles at a minimum.  Now discretise Eq.~(\ref{eq:4-heavyball}) with
a time step $\Delta t$, writing $\bm{\theta}_k=\bm{\theta}(k\Delta t)$ and
replacing the derivatives by finite differences,
\begin{equation*}
  \frac{d^{2}\bm{\theta}}{dt^{2}}\approx
  \frac{\bm{\theta}_{k+1}-2\bm{\theta}_k+\bm{\theta}_{k-1}}{(\Delta t)^{2}},
  \qquad
  \frac{d\bm{\theta}}{dt}\approx
  \frac{\bm{\theta}_{k+1}-\bm{\theta}_k}{\Delta t}.
\end{equation*}
Multiplying through by $(\Delta t)^{2}$ and collecting the terms in
$\bm{\theta}_{k+1}$ on the left,
\begin{equation*}
  (m+\mu\Delta t)\,(\bm{\theta}_{k+1}-\bm{\theta}_k)
  = m\,(\bm{\theta}_k-\bm{\theta}_{k-1})
    - (\Delta t)^{2}\,\nabla C(\bm{\theta}_k),
\end{equation*}
so that the position update is
\begin{equation}
  \bm{\theta}_{k+1} = \bm{\theta}_k
  + \underbrace{\frac{m}{m+\mu\Delta t}}_{\beta}\,
    (\bm{\theta}_k-\bm{\theta}_{k-1})
  - \underbrace{\frac{(\Delta t)^{2}}{m+\mu\Delta t}}_{\gamma}\,
    \nabla C(\bm{\theta}_k) .
  \label{eq:4-heavyballdiscrete}
\end{equation}
With $\bm{v}_{k+1}=\bm{\theta}_k-\bm{\theta}_{k+1}$ this is exactly
Eq.~(\ref{eq:4-momentum}): the new displacement is $\beta$ times the old
one plus $\gamma$ times the gradient.  The momentum parameter is the
fraction of its velocity the particle keeps from one step to the next,
and the learning rate is the squared time step divided by the effective
mass.  (Many texts and the slides of this course write the same iteration
as $\bm{v}_{k}=\beta(\bm{\theta}_k-\bm{\theta}_{k-1})-\gamma\nabla
C(\bm{\theta}_k)$, $\bm{\theta}_{k+1}=\bm{\theta}_k+\bm{v}_k$; that is
Eq.~(\ref{eq:4-momentum}) with the sign of $\bm{v}$ reversed, and nothing
else changes.)

The two limits of Eq.~(\ref{eq:4-heavyballdiscrete}) are instructive.  When
friction dominates, $\mu\Delta t\gg m$, we have $\beta\to0$ and
$\gamma\to\Delta t/\mu$: the particle has no inertia, its velocity is
proportional to the force, and the iteration is plain gradient descent -- a
massless bead sliding through treacle.  When friction is negligible,
$\mu\Delta t\ll m$, we have $\beta\to1$ and $\gamma\to(\Delta t)^{2}/m$:
energy is conserved, the particle never settles, and the iteration rings
across the valley indefinitely.  This is the frictionless ``ball'' of the
example below, where $\beta=0.9$ overshoots the global minimum.  Between the
two, $1-\beta=\mu\Delta t/(m+\mu\Delta t)$ measures how much energy is
dissipated per step.  The gain $1/(1-\beta)$ that we shall meet in
Eq.~(\ref{eq:4-momentumgain}) below is the terminal velocity of the particle: under a constant force $\bm{g}$
the steady displacement per step is $\gamma\bm{g}/(1-\beta)=(\Delta t/\mu)\,
\bm{g}$, that is, the velocity $\bm{g}/\mu$ at which friction exactly
balances the force, independent of the mass.  What a heavier ball buys is
not a higher terminal velocity but a longer memory: it takes about
$1/(1-\beta)$ steps to forget a gradient, which is what averages out the
sign-alternating gradients of a steep direction.

The method was introduced under the name \emph{heavy-ball method} by
Polyak~\cite{polyak1964}, who also derived the rate quoted below; the
reading as a discretised equation of motion, and the limits above, follow
Qian~\cite{qian1999}.
...
Section~\ref{sec:learningrate}.  Polyak's analysis of the two-term recurrence~\cite{polyak1964}
...
\emph{Nesterov} momentum~\cite{nesterov1983,sutskever2013}, which evaluates the gradient not at
...
standard remedy is the last: let $\gamma$ decay with time.  Let $e=0,1,2,\dots$ index the epoch, let $m$ be the
...
\]
so that $g'_j=c\,g_j$ while $g'_k=g_k$ for $k\neq j$,
...
$t$.  Two consequences follow.  The quantity $\bm{v}_t$ is now an estimate of the
...
of $\rho$, and the excess cost floors at a value of order $\lambda\gamma^{2}$.  On the
...
\paragraph{The size of an Adam step.}
...
The same five updates can be written as a single \emph{step function} that
...
%......................................................................
\subsection{Two examples worked by hand}
\label{sec:adexamples}

The example of Eq.~(\ref{eq:4-adexample}) had two inputs, which is what
makes the difference between the two modes visible.  It is nevertheless
worth running both sweeps on two functions of a single variable, small
enough that every number can be checked against the derivative one would
write down by hand, because they show the two things a first encounter with
AD tends to obscure: that the sweeps only ever multiply and add numbers
already on the tape, and that the reverse sweep has to \emph{sum} whenever a
value is used more than once.

\paragraph{A chain.}
Take first
\begin{equation}
  f(x)=\exp\bigl(x^{2}\bigr),
  \label{eq:4-adchainfun}
\end{equation}
whose trace has a single input and two intermediate values,
\begin{equation}
  v_0 = x,\qquad v_1 = v_0^{2},\qquad v_2 = \exp v_1,\qquad y = v_2 ,
  \label{eq:4-adchaintrace}
\end{equation}
with local derivatives $\partial v_1/\partial v_0=2v_0$ and
$\partial v_2/\partial v_1=\exp v_1=v_2$.  The computational graph, upper
panel of Fig.~\ref{fig:4-adchain}, is a chain: every node has one
predecessor and one successor.  The forward recursion~(\ref{eq:4-forwardrec})
seeded with $\dot v_0=1$ reads
\begin{equation}
  \dot v_1 = 2v_0\,\dot v_0 = 2x,\qquad
  \dot v_2 = v_2\,\dot v_1 = 2x\exp\bigl(x^{2}\bigr),
  \label{eq:4-adchainforward}
\end{equation}
and the reverse recursion~(\ref{eq:4-reverserec}) started from $\bar v_2=1$
reads
\begin{equation}
  \bar v_1 = \bar v_2\,v_2 = \exp\bigl(x^{2}\bigr),\qquad
  \bar v_0 = \bar v_1\,2v_0 = 2x\exp\bigl(x^{2}\bigr).
  \label{eq:4-adchainreverse}
\end{equation}
Both are the chain rule $df/dx=(df/dv_2)(dv_2/dv_1)(dv_1/dv_0)$; forward
mode multiplies the three factors from the right, reverse mode from the
left, and on a chain the two are indistinguishable in cost.  Evaluating
$f$ takes two elementary operations, a square and an exponential.  Either
sweep adds two multiplications, and notice that $\exp(x^{2})$ is not
recomputed: it is $v_2$, read off the tape.  At $x=1$ all three agree on
$f'(1)=2e=5.437$.
...
\begin{tikzpicture}[>=Latex,x=1cm,y=1cm,
  ...
  outp/.style={circle,draw=black!65,fill=Red3!12,minimum size=8.5mm,
               inner sep=0pt,font=\small},
  lab/.style={font=\footnotesize,black!60}]
  % top panel: the chain exp(x^2)
  \node[inp] (a0) at (0,0) {$v_0$};
  \node[op]  (a1) at (2.0,0) {$v_1$};
  \node[outp](a2) at (4.0,0) {$v_2$};
  \draw[->,black!70] (a0) -- node[above,lab] {$(\cdot)^{2}$} (a1);
  \draw[->,black!70] (a1) -- node[above,lab] {$\exp$} (a2);
  \draw[->,black!70] (a2) -- ++(1.2,0) node[right,font=\small] {$y$};
  \node[lab] at (0,-0.85) {$x$};
  \node[lab,anchor=west] at (6.2,0) {$f(x)=\exp(x^{2})$};
  % bottom panel: the fan-out sqrt(x^2 + exp(x^2))
  \begin{scope}[yshift=-3.3cm]
  \node[inp] (b0) at (0,0) {$v_0$};
  \node[op]  (b1) at (2.0,0) {$v_1$};
  \node[op]  (b2) at (4.0,1.3) {$v_2$};
  \node[op]  (b3) at (6.0,0) {$v_3$};
  \node[outp](b4) at (8.0,0) {$v_4$};
  \draw[->,black!70] (b0) -- node[above,lab] {$(\cdot)^{2}$} (b1);
  \draw[->,black!70] (b1) -- node[above left,lab] {$\exp$} (b2);
  \draw[->,black!70] (b1) -- node[below,lab] {$+$} (b3);
  \draw[->,black!70] (b2) -- (b3);
  \draw[->,black!70] (b3) -- node[above,lab] {$\sqrt{\;}$} (b4);
  \draw[->,black!70] (b4) -- ++(1.2,0) node[right,font=\small] {$y$};
  \node[lab] at (0,-0.85) {$x$};
  \node[lab] at (4.0,-1.4) {$f(x)=\sqrt{x^{2}+\exp(x^{2})}$};
  \end{scope}
\end{tikzpicture}
\caption{The computational graphs of the two examples of
Section~\ref{sec:adexamples}.  Top: $f(x)=\exp(x^{2})$, trace
(\ref{eq:4-adchaintrace}), a chain in which every value is used once.
Bottom: $f(x)=\sqrt{x^{2}+\exp(x^{2})}$, trace (\ref{eq:4-adfanouttrace}),
where $v_1=x^{2}$ feeds both the exponential and the sum.  The two paths
from $v_1$ to $y$ are the two terms of the adjoint $\bar v_1$ in
Eq.~(\ref{eq:4-adfanoutreverse}).}
\label{fig:4-adchain}
\end{figure}

\paragraph{A fan-out.}
The second function reuses an intermediate value,
\begin{equation}
  f(x)=\sqrt{x^{2}+\exp\bigl(x^{2}\bigr)} ,
  \label{eq:4-adfanoutfun}
\end{equation}
with the trace
\begin{equation}
  v_0 = x,\qquad v_1 = v_0^{2},\qquad v_2 = \exp v_1,\qquad
  v_3 = v_1 + v_2,\qquad v_4 = \sqrt{v_3},\qquad y = v_4 ,
  \label{eq:4-adfanouttrace}
\end{equation}
shown in the lower panel of Fig.~\ref{fig:4-adchain}.  The node $v_1$ now
has two successors, $v_2$ and $v_3$, and this is where the two modes part
company.  The forward sweep is unaffected -- each node still collects
tangents from its predecessors, and $v_3$ simply has two of them:
\begin{equation}
  \dot v_1 = 2v_0,\qquad
  \dot v_2 = v_2\,\dot v_1,\qquad
  \dot v_3 = \dot v_1 + \dot v_2 = (1+v_2)\,\dot v_1,\qquad
  \dot v_4 = \frac{\dot v_3}{2v_4} .
  \label{eq:4-adfanoutforward}
\end{equation}
The reverse sweep starts from $\bar v_4=1$ and walks back through the
trace,
\begin{equation}
  \begin{aligned}
  \bar v_3 &= \frac{\bar v_4}{2v_4} = \frac{1}{2\sqrt{v_3}}, &
  \bar v_2 &= \bar v_3\cdot 1,\\
  \bar v_1 &= \bar v_2\,v_2 + \bar v_3\cdot 1 = \frac{1+v_2}{2\sqrt{v_3}}, &
  \bar v_0 &= \bar v_1\,2v_0 = \frac{x\,(1+v_2)}{\sqrt{v_3}} ,
  \end{aligned}
  \label{eq:4-adfanoutreverse}
\end{equation}
and the adjoint of $v_1$ is a \emph{sum} of two terms, one through each
successor, exactly as Eq.~(\ref{eq:4-reverserec}) prescribes: the
sensitivity of $y$ to $v_1$ is the sensitivity through the exponential plus
the sensitivity through the direct addition.  Substituting $v_2=\exp(x^{2})$
and $v_3=x^{2}+\exp(x^{2})$ gives
\begin{equation}
  f'(x) = \frac{x\,\bigl(1+\exp(x^{2})\bigr)}{\sqrt{x^{2}+\exp(x^{2})}} ,
  \label{eq:4-adfanoutclosed}
\end{equation}
which is what the chain rule applied by hand produces.  At $x=1$ the
tape holds $v_1=1$, $v_2=2.718$, $v_3=3.718$ and $v_4=1.928$, and both
sweeps return $f'(1)=3.718/1.928=1.928$.  (That $f'(1)$ equals $f(1)$ is a coincidence of
the point $x=1$, where the numerator $x(1+v_2)$ happens to equal $v_3$.)

The bookkeeping is worth comparing.  Evaluating $f$ costs four elementary
operations.  The reverse sweep adds two for $\bar v_3$, two for $\bar v_1$
and two for $\bar v_0$, six in all, and every quantity it needs --
$v_2$, $v_4$, $v_0$ -- is already on the tape.  Evaluating the closed
form~(\ref{eq:4-adfanoutclosed}) on its own, without sharing anything with
the evaluation of $f$, costs about eight operations and computes $x^{2}$
and $\exp(x^{2})$ a second time; on a function with thousands of
intermediate values the hand-derived expression grows far faster than the
trace, which is the \emph{expression swell} that AD avoids by never forming
it.  Both sweeps here are within the constant of
Proposition~\ref{prop:4-cheapgradient}, and the reader can check that the
same holds for the example of Fig.~\ref{fig:4-compgraph}.

The following listing runs the two examples through the \texttt{Dual} class
of Section~\ref{sec:adforward} for the forward sweep and through JAX for
the reverse sweep (Section~\ref{sec:adpython} says more about JAX), and
compares both with Eq.~(\ref{eq:4-adfanoutclosed}).  The functions are
written exactly as one would write them without derivatives in mind.
...
The three numbers agree to rounding, $1.92828468553247$, differing at most
in the last binary digit: nothing was approximated in either sweep.
...
\begin{itemize}
  ...
  \item \texttt{autodiff\_examples.py} -- the \texttt{Dual} class of
        Section~\ref{sec:adforward} and the two hand-worked examples of
        Section~\ref{sec:adexamples}, the \texttt{autograd} and \texttt{JAX}
...
\item \textbf{The minibatch gradient as an estimator.}
