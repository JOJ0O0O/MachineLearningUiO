...
The class above hard-wires the gradient~(\ref{eq:5-gradient}), which we
derived by hand in Section~\ref{sec:logreggradient}.  Logistic regression is
...
Fig.~\ref{fig:5-logreggraph}.  Its one structural feature is that the linear
...
squares, Eq.~(\ref{eq:5-irls}) -- for small ones.
